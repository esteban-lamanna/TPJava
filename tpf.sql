...
#
# Structure for the `lineacarro` table : 
#

DROP TABLE IF EXISTS `lineacarro`;

CREATE TABLE `lineacarro` (
  `codigo_producto` int(11) NOT NULL,
  `codigo_carro` int(11) NOT NULL auto_increment,
  PRIMARY KEY  (`codigo_carro`,`codigo_producto`),
  KEY `codigo_producto` (`codigo_producto`),
  KEY `codigo_carro` (`codigo_carro`),
  CONSTRAINT `lineacarro_carro_fk` FOREIGN KEY (`codigo_carro`) REFERENCES `carritos` (`codigo`) ON UPDATE CASCADE,
  CONSTRAINT `lineacarro_fk` FOREIGN KEY (`codigo_producto`) REFERENCES `productos` (`codigo`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

#
# Data for the `lineacarro` table  (LIMIT 0,500)
#

INSERT INTO `lineacarro` (`codigo_producto`, `codigo_carro`) VALUES 
  (19,4);
